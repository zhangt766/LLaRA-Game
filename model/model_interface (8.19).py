...
score_instruct = """You are a system that recommends games based on play history. Please evaluate the similarity between each play history in the candidate list and the single target play history. Rate the similarity on a scale from 1 to 10 between , where 1 is not similar at all and 10 is very similar.\n
...
score_history = """Candidate Play History:
{MOVIE_LISTS} \n
Target Play History:
{TARGET_MOVIE} \n
...
reco_instruct = """You are a game recommendation system. Below are some similar users' play histories and the next game they are likely to choose. Based on the current user's play history, your task is to recommend the next game for this user. Instructions:1. Recommend one game title. 2. It **must** be from the candidate pool only.
Please output the recommendation in the format below:
['Recommendation': game_name] \n
"""

reco_prompt_history = """Similar user {i}: He/She has palyed {SimilarHistory}. Based on this, He/She will likely choose {SimilarChoice} to play next. \n"""

reco_prompt_instruct = """The play history of this user is: {HistoryHere}. Recommend one game from the following set of titles: {CansHere}. Output:"""
...
class MInterface(pl.LightningModule):
    ...
    def score_demo(self, input):
        ...
        input_prompt = score_instruct + score_history.format_map(
            {"GAME_LISTS": movie_lists, "TARGET_GAME": target_movie})
